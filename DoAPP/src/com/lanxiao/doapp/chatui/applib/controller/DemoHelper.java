package com.lanxiao.doapp.chatui.applib.controller;
...
import com.lanxiao.doapp.chatui.applib.chatuimain.utils.EmojiconExampleGroupData;
import com.lanxiao.doapp.chatui.applib.chatuimain.utils.UserProfileManager;
import com.lanxiao.doapp.chatui.applib.model.DemoModel;
import com.lanxiao.doapp.chatui.applib.utils.PreferenceManager;
...
public class DemoHelper {
    ...
    public void setDoUser(DoUser doUser) {
        this.doUser = doUser;
    }
    public DoUser getDoUser() {
        return doUser;
    }
    /**
     * 数据同步listener
    ...
    class MyGroupChangeListener implements EMGroupChangeListener {
        ...
        public void onInvitationReceived(String groupId, String groupName, String inviter, String reason) {
           ...
           setNickNameandAver(inviter,msg,st3);
        }
         ...
         * @param from
         */
        public void setNickNameandAver(String from, final EMMessage msg, final String st3){
            LogUtils.i(from + "setNickNameandAver");
            HttpUtils hu=new HttpUtils();
            RequestParams rp=new RequestParams("UTF-8");
            rp.addBodyParameter("douserid",from);
            hu.send(com.lidroid.xutils.http.client.HttpRequest.HttpMethod.POST, SEARCH_FRIEND, rp, new RequestCallBack<String>() {
                @Override
                public void onSuccess(ResponseInfo<String> responseInfo) {
                    try {
                        LogUtils.i(responseInfo.result);
                        JSONObject js=new JSONObject(responseInfo.result);
                        String ToNickName=js.optString("username");
                        String heanUrl=js.optString("userheadlogo");
                        if(js.optString("message").equals("成功")){
                            msg.addBody(new TextMessageBody(ToNickName + " " + st3));
                            // 保存邀请消息
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactDeleted(final List<String> usernameList) {
            ...
            for (String username : usernameList) {
                ...
                try {
                    //todo
                 // EaseUser first = DemoApplication.getInstance().getDb().findFirst(Selector.from(EaseUser.class).where("userid", "=", username));
                    //DemoApplication.getInstance().getDb().delete(first);
                    DemoApplication.getInstance().getDb().delete(EaseUser.class, WhereBuilder.b("userId", "==", username).and("whosfriend", "==", DemoHelper.getInstance().getCurrentUsernName()));
                    Log.i("weitie", "删除:" + username);
                } catch (DbException e) {
                    e.printStackTrace();
                    Log.i("weitie","删除失败:"+username);
                }
            }
            broadcastManager.sendBroadcast(new Intent(Constant.ACTION_CONTACT_CHANAGED));
        }
    ...
    private EaseUser getUserInfo(String username) {
        ...
        EaseUser user = null;
        LogUtils.i("username"+(username==null));
        LogUtils.i("EMChatManager"+(EMChatManager.getInstance().getCurrentUser()));
    ...
    synchronized void reset() {
      ...
      /*  try {
            DemoApplication.getInstance().getDb().deleteAll(EaseUser.class);
        } catch (DbException e) {
            e.printStackTrace();
        }*/
    }
