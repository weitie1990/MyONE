...
public class ArcLayout extends ViewGroup {
    ...
    private  int MIN_RADIUS = 460;//调节弹出的距离
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        final int radius = mRadius = computeRadius(Math.abs(mToDegrees - mFromDegrees), getChildCount(), mChildSize,
                mChildPadding, AutoUtils.getPercentHeightSize(MIN_RADIUS));
        final int size = radius * 2 + mChildSize + mChildPadding + mLayoutPadding * 2;
        //
        setMeasuredDimension(size + 60, size + 60);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        Log.i("weitie","mExpanded:"+mExpanded);
    ...
    private static Animation createExpandAnimation(float fromXDelta, float toXDelta, float fromYDelta, float toYDelta,
        ...
        Animation animation = new com.lanxiao.doapp.capricorn.RotateAndTranslateAnimation(0, toXDelta, 0, toYDelta, 0, 720);
    ...
    private static Animation createShrinkAnimation(float fromXDelta, float toXDelta, float fromYDelta, float toYDelta,
        ...
        Animation translateAnimation = new com.lanxiao.doapp.capricorn.RotateAndTranslateAnimation(0, toXDelta, 0, toYDelta, 360, 720);
    ...
    public void switchState(final boolean showAnimation) {
        if (showAnimation) {
            final int childCount = getChildCount();
            for (int i = 0; i < childCount; i++) {
                bindChildAnimation(getChildAt(i), i, 600);
            }
        }
        cwjHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                onAllAnimationsEnd();
            }
