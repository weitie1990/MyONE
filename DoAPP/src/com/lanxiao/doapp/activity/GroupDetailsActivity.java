...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new Thread()
		{
			public void run()
			{
				...
				try {
					...
					e.printStackTrace();
					Utils.showToast("无网络",getApplicationContext());
				}
			}
		}.start();
		...
		LogUtils.i(group.getMembers().size() + "");
		LogUtils.i(members.size()+"");
		if(members.size()!=0) {
	 ...
	 * @param sb
	 */
	public  void getMachFriend2(String sb) {
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {
				...
				try {
					...
					for (int i = 0; i < ja.length(); i++) {
						...
						user.setAvatar(js2.optString("touxiang"));
						user.setNickName(js2.optString("nickname"));
						LogUtils.i(js2.optString("nickname"));
						user.setInitialLetter(Utils.getPingYin(js2.optString("nickname")));
						contactList.add(user);
					}
					adapter = new GridAdapter(getApplicationContext(), R.layout.grid, members,contactList);
	...
	private class GridAdapter extends ArrayAdapter<EaseUser> {
		...
		public GridAdapter(Context context, int textViewResourceId, List<String> objects,List<EaseUser> contactList) {
...
//				button.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.smiley_add_btn, 0, 0);
				...
				final String username =contactList.get(position).getUserId();
				convertView.setVisibility(View.VISIBLE);
...
//				button.setCompoundDrawables(null, avatar, null, null);
				...
				if(!TextUtils.isEmpty(contactList.get(position).getAvatar())){
					//BitmapUtils bp=new BitmapUtils(getContext());
					Utils.setAver(contactList.get(position).getAvatar(),holder.imageView);
				}
				LogUtils.i("getNickName:"+contactList.get(position).getNickName());
				holder.textView.setText(contactList.get(position).getNickName());
				if (isInDeleteMode) {
					// 如果是删除模式下，显示减人图标
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					// 更新本地数据
					group=EMGroupManager.getInstance().createOrUpdateLocalGroup(returnGroup);
