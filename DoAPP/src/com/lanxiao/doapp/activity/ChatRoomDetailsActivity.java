...
import com.lanxiao.doapp.chatui.applib.chatuimain.utils.EaseUserUtils;
...
public class ChatRoomDetailsActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		LogUtils.i(owner.size()+"");
		if(owner.size()!=0) {
			StringBuffer sb = new StringBuffer();
			for (int i = 0; i < owner.size(); i++) {
				sb.append(owner.get(i) + ",");
			}
		...
		httpUtils.send(com.lidroid.xutils.http.client.HttpRequest.HttpMethod.POST, MyUtils.SEARCH_ALL_FRIEND, params, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {
				...
				try {
					...
					adapter = new GridAdapter(getApplicationContext(), R.layout.grid, owner,contactList);
...
//									REQUEST_CODE_ADD_USER);
				...
				if(!TextUtils.isEmpty(contactList.get(position).getAvatar())){
					...
					bp.display(holder.imageView,contactList.get(position).getAvatar());
				}else {
					...
					public void onClick(View v) {
						if (isInDeleteMode) {
							...
							if (EMChatManager.getInstance().getCurrentUser().equals(username)) {
								startActivity(new Intent(ChatRoomDetailsActivity.this, EaseAlertDialog.class).putExtra("msg", st12));
								return;
							}
